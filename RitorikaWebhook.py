...
class DISCTest:
    ...
    def finish_test(self, chat_id):
        text = f'Спасибо за прохождение теста!\n\n✅Ваш результат: {int(self.test_score / 46 * 100)}%\n\n' \
...
class SimpleUserTestDISC:
    def __init__(self, msg, bot_ref, texts):
        ...
        self.try_times = 0
        self.id = msg.chat.id
        self.test = DISCTest('TEST1.xlsx', bot_ref, texts)
        self.auth(msg)
    ...
    def get_code(self, msg):
        ...
        if msg.text == key:
            with open('logger.txt', 'a') as logger:
                logger.write(f'[Тестобот "Риторика"]: Пользователь с id: {msg.chat.id} Авторизация прошла успешно!'
            ...
            if self.try_times <= 5:
                ...
                with open('logger.txt', 'a') as logger:
                    logger.write(f'[Тестобот "Риторика"]: Пользователь с id: {msg.chat.id} Ошибка авторизации!  - '
                                 f'{datetime.datetime.today()}\n')
    ...
    def callback_handler(self, call):
        if call.data == 'CALL_DISC':
            ...
            self.test = DISCTest('TEST1.xlsx', self.bot, self.test.texts)
...
API_TOKEN = os.getenv('API_TOKEN_RITORIKA_TEST')
...
texts_literal = {
        ...
        'START': 'Добрый день 👋🏼\n\nГотовы пройти тест на знания  ""Профессиональная риторика за столом"?'
    }
