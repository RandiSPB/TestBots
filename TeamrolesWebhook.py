...
class QBlock:
    ...
    def start(self, chat_id):
        text = '<b>{0}\n{1}</b> {2}'.format(self.block_title, self.questions[self.hypo_counter][0], self.questions[self.hypo_counter][2])
    ...
    def process_answer(self, call):
        ...
        try:
            ...
            self.bot.send_message(call.message.chat.id, 'Простите, возникла ошибка! С целью сохранения ваших результатов. Не могли бы вы пройти блок еще раз :)')
            self.change(call.message.chat.id)
    ...
    def get_code(self, msg):
        ...
        if msg.text == key:
            with open('logger.txt', 'a') as logger:
                logger.write(f'[Тестобот "Роль в команде"]: Пользователь с id: {msg.chat.id} Авторизация прошла успешно!'
            ...
            if self.try_times <= 5:
                ...
                with open('logger.txt', 'a') as logger:
                    logger.write(f'[Тестобот "Роль в команде"]: Пользователь с id: {msg.chat.id} Ошибка авторизации!  - '
                                 f'{datetime.datetime.today()}\n')
...
API_TOKEN = os.getenv('API_TOKEN_TEAMROLES')
...
def handler(message):
    if message.text == '/start':
        try:
            ...
            users[message.chat.id] = SimpleUserTestTeamwork(message, bot)
...
def smo(call):
    try:
        ...
        users[call.message.chat.id] = SimpleUserTestTeamwork(call.message, bot)
