...
class DISCTest:
    ...
    def handler(self, call):
        if call.data != 'NO_CB':
            answer_types = call.data.split(',')
            for answer_type in answer_types:
                self.types_scores[answer_type] += 1
            tmp = self.questions[tuple(self.questions.keys())[self.cur_q - 1]]
            index = tmp['scores'].index(call.data)
            print(index)
    ...
    def send_question(self, chat_id):
        try:
            ...
            print(q_data)
            tmp = self.questions[q_data]['answers']
            print(tmp)
            print(self.questions[q_data]['scores'])
            if self.cur_q == 7 or self.cur_q == 10:
                question_text = '<b>{0} из 15\n\n{1}</b>\n\n{2}\n\n{3}'.format(self.cur_q, q_data, *tmp)
                button_labels = ('А', 'Б')
            else:
                question_text = '<b>{0} из 15\n\n{1}</b>\n\n{2}\n\n{3}\n\n{4}\n\n{5}'.format(self.cur_q, q_data, *tmp)
    ...
    def finish_test(self, chat_id):
        with open('content/psyho-types.jpg', 'rb') as psyho_types_image:
...
API_TOKEN = os.getenv('API_TOKEN_DISC')
